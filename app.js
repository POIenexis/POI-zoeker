...
function links(i){let lat=i.lat,lon=i.lon,enc=encodeURIComponent([i.street,i.house,i.zip,i.city].filter(Boolean).join(" "));return{apple:`https://maps.apple.com/?daddr=${lat},${lon}&dirflg=d`,google:`https://www.google.com/maps/dir/?api=1&destination=${lat},${lon}`,waze:`https://waze.com/ul?ll=${lat}%2C${lon}&navigate=yes`,tomtom:`tomtomgo://x-callback-url/navigate?destination=${lat},${lon}`,geo:`geo:${lat},${lon}?q=${lat},${lon}(${enc})`}}
...
function locate(fromSettings=false){if(!navigator.geolocation)return toast("Locatie niet beschikbaar");$("#status").textContent="GPS-toestemming en locatie opvragen…";navigator.geolocation.getCurrentPosition(p=>{mode="normal";setPos(p,false,true);if(fromSettings)$("#settingsDlg").close()},e=>{toast(e.code===1?"Locatietoegang geblokkeerd — sta GPS toe in de site-instellingen":"Locatie kon niet worden bepaald");render()},{enableHighAccuracy:true,timeout:15000,maximumAge:0})}
...
load();if(localStorage.getItem("locationEnabled")==="1")startSmartLocation();else window.addEventListener("load",()=>setTimeout(()=>locate(false),500));
